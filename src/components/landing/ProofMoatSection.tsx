...
export function ProofMoatSection({
  ...
  return (
    <section className="grid w-full min-w-0 gap-6 lg:grid-cols-[0.95fr_1.05fr]">
      <div className="min-w-0 rounded-[28px] border border-white/10 bg-white/[0.03] p-6 sm:p-8">
      ...
      <div className="min-w-0 overflow-hidden rounded-[28px] border border-cyan-300/16 bg-slate-950/72 p-6 shadow-[0_24px_120px_rgba(0,0,0,0.35)]">
        <div className="flex items-center justify-between">
          ...
          <div className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-[11px] text-slate-300">
            {replay?.deterministicMatch ? 'replay verified' : 'live replay unavailable'}
          </div>
        </div>
        ...
        <div className="mt-6 grid min-w-0 gap-4 sm:grid-cols-2">
          <div className="min-w-0 rounded-2xl border border-white/8 bg-white/[0.03] p-4">
            ...
            <div className="mt-3 space-y-2 text-sm text-slate-300">
              <div>job_id: {proof?.job_id ?? 'live data unavailable'}</div>
              <div>selected_region: {proof?.selected_region ?? 'live data unavailable'}</div>
              <div>proof_hash: {proof?.proof_hash?.slice(0, 18) ?? 'live data unavailable'}</div>
              <div>confidence: {proof?.confidence_score?.toFixed(2) ?? 'live data unavailable'}</div>
            </div>
          </div>
          <div className="min-w-0 rounded-2xl border border-white/8 bg-white/[0.03] p-4">
            ...
            <div className="mt-3 text-xs text-slate-400">
              replay {replay?.deterministicMatch ? 'deterministic match' : 'pending live frame'}
            </div>
          </div>
        </div>
      </div>
