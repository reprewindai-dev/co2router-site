...
export function LiveRoutingDemo({ externalRunSignal = 0 }: LiveRoutingDemoProps) {
  ...
  const runScenario = useStableCallback((id: HeroScenarioId) => {
    ...
    fetch('/api/decision', {
      ...
      .then((r) => r.json())
      .then((data: ApiDecision) => {
        if (!signal.aborted) setLiveDecision(data)
      })
      .catch(() => {
        if (!signal.aborted) {
          setLiveDecision({
  ...
  const displayRegion =
    liveDecision?.routedRegion || liveDecision?.selectedRegion || scenario.requestedRegion
  const displayLatency = liveDecision?.latencyMs ?? 77

  const statusLabel = phase < 3 ? 'SIGNAL EVALUATION' : phase < 5 ? 'GOVERNANCE CHAMBER' : 'READY'

  return (
    <section id="hero-live-demo" className="lg:min-h-[720px]">
      <div
        className={`h-full rounded-[36px] border border-white/10 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.09),transparent_32%),linear-gradient(180deg,rgba(13,17,24,0.97),rgba(4,6,12,0.99))] p-4 transition-all duration-700 ${tone.glow} sm:p-5`}
      >
        <div className="flex h-full flex-col rounded-[30px] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.035),rgba(255,255,255,0.012))] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.055)] sm:p-5">
          <div className="flex items-center justify-between gap-4 border-b border-white/8 pb-4">
            <div className="space-y-0.5">
              ...
              <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] text-slate-500">
                <span className="inline-flex h-2 w-2 animate-pulse rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.7)]" />
          ...
          <div className="mt-4 flex flex-col gap-4">
            <div
              className={`rounded-[26px] border border-white/10 bg-slate-950/70 p-4 transition-all duration-600 ${phase >= 1 ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0'}`}
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="space-y-1 text-right text-sm text-slate-400">
                  <div>
                    Source: <span className="text-slate-200">{scenario.source}</span>
                  </div>
                  <div>
                    Region: <span className="text-slate-200">{scenario.requestedRegion}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-5 gap-2">
              {scenario.signals.map((sig, i) => (
                <div
                  ...
                  className={`rounded-[20px] border border-white/8 bg-white/[0.028] p-3 transition-all duration-600 ${phase >= 2 ? 'translate-y-0 opacity-100' : 'translate-y-1 opacity-0'}`}
                >
                  <div className="text-[9px] uppercase tracking-[0.22em] text-slate-500">
                    {sig.label}
                  </div>
                  <div className="mt-1.5 text-xs font-semibold leading-5 text-white">
                    {sig.value}
                  </div>
                </div>
              ))}
            ...
            <div
              className={`rounded-[26px] border border-white/10 bg-[linear-gradient(180deg,rgba(11,15,23,0.96),rgba(5,8,14,0.99))] p-5 transition-all duration-700 ${phase >= 4 ? 'scale-100 opacity-100' : 'scale-[0.99] opacity-60'}`}
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div
                    className={`mt-2 text-5xl font-black tracking-[-0.06em] text-white transition-all duration-500 sm:text-6xl ${phase >= 5 ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}`}
                  >
                    {displayAction}
                  </div>
                ...
                <div className={`rounded-2xl px-4 py-3 text-right ${tone.badge}`}>
                  ...
                  <div className="mt-1 text-xs font-bold uppercase tracking-[0.18em]">
                    {displayAction}
                  </div>
                </div>
              </div>
              ...
              {phase >= 6 && (
                <div className="mt-4 rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-3 text-sm leading-7 text-slate-300">
                  ...
                  {displayRegion && activeId === 'reroute' && (
                    <span className="ml-2 text-slate-400">to {displayRegion}</span>
                  )}
                </div>
              )}
            </div>
            ...
            {phase >= 6 && (
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-[24px] border border-emerald-400/15 bg-emerald-400/8 px-4 py-3">
                <div className="text-[10px] uppercase tracking-[0.26em] text-emerald-300">
                  Proof strip
                </div>
                <div className="flex flex-wrap gap-2 text-[11px] text-slate-200">
                  {scenario.proof.slice(0, 3).map((p) => (
                    <span
                      key={p}
                      className="rounded-xl border border-white/8 bg-slate-950/55 px-2.5 py-1"
                    >
                      {p}
                    </span>
                  ))}
                  <span className="rounded-xl border border-white/8 bg-slate-950/55 px-2.5 py-1">
                    {displayLatency}ms p95
                  </span>
                </div>
              </div>
            )}
