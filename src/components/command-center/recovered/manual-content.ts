...
export const HALOGRID_MANUAL_SECTIONS: HalogridManualSection[] = [
  {
    ...
    paragraphs: [
      'HaloGrid is the CO2 Router command-center surface for deterministic pre-execution governance. Compute does not run until it is authorized. The system issues a binding decision before execution, not a recommendation after the fact.',
  ...
  },
  {
    ...
    title: 'HUD Elements',
    paragraphs: [
      'The HUD is always driven from the live HaloGrid snapshot. It summarizes fleet health, carbon pressure, blocked posture, decision velocity, queue pressure, provider stress, and current operator confidence without requiring additional clicks.',
    ],
    bullets: [
