...
export function getContactRoutingConfig() {
  return {
    sales: process.env.CONTACT_SALES_EMAIL?.trim() || null,
    support: process.env.CONTACT_SUPPORT_EMAIL?.trim() || null,
    security: process.env.CONTACT_SECURITY_EMAIL?.trim() || null,
  }
}
